...
const ChatBot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* 气泡引导 */}
      {!isOpen && (
        <div className="absolute -left-32 -top-8 bg-white rounded-2xl shadow-xl px-4 py-2 flex items-center animate-bounce" style={{ animationDuration: '3s', animationIterationCount: 'infinite', animationTimingFunction: 'ease-in-out' }}>
      ...
      {/* 聊天框 */}
      {(isOpen || isAnimating) && (
        <div className={`absolute bottom-20 right-0 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden transform transition-all duration-500 cubic-bezier(0.68, -0.55, 0.265, 1.55) ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
          {/* 聊天框头部 */}
          <div className="bg-white p-4 flex items-center justify-between border-b border-gray-100">
            <div className="flex items-center space-x-3">
              <img src="/images/avatar.png" alt="Rose" className="w-9 h-9 rounded-full object-cover" />
              <div>
          ...
          <div className="p-4 h-80 overflow-y-auto bg-gray-50">
            {messages.map((message) => (
              ...
              >
                ...
                <div
                  className={`max-w-[80%] px-4 py-2 ${message.isUser 
